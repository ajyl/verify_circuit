...
def _build_inner_batch(recording, timesteps_not, timesteps_this):
    ...
    _resid_streams_per_layer = {}
    for layer_idx, (module_name, acts) in enumerate(recording.items()):
        _labels = []
        for b_idx in timesteps_not.keys():
            _not_timesteps = timesteps_not[b_idx].tolist()

            # [n_samples, n_heads, d_model]
            _acts = torch.cat(acts, dim=1)[b_idx, _not_timesteps]
            if layer_idx not in _resid_streams_per_layer:
                _resid_streams_per_layer[layer_idx] = []
            _resid_streams_per_layer[layer_idx].append(_acts)
            _labels.extend([0] * len(_not_timesteps))

            _this_timesteps = timesteps_this[b_idx].tolist()
            _acts = torch.cat(acts, dim=1)[b_idx, _this_timesteps]
            _resid_streams_per_layer[layer_idx].append(_acts)
    ...
    # map {layer_idx: [batch, n_heads, d_model]}
    _resid_streams_per_layer = {
        layer_idx: torch.cat(acts, dim=0)
        for layer_idx, acts in _resid_streams_per_layer.items()
    }

    # [batch, n_layers, n_heads, d_model]
    _resid_streams = torch.stack(list(_resid_streams_per_layer.values()), dim=1)
    assert len(_labels) == _resid_streams.shape[0]
...
def cache_valid_data(model, tokenizer, data_loader, config):
            ...
            for b_idx, text in enumerate(response_text):
                ...
                if len(not_matches) > 0 and len(this_matches) > 0:
                    filtered_timesteps_not[b_idx] = torch.tensor(
                        list(set([r["token_index"] for r in not_matches]))
                    )
                    filtered_timesteps_this[b_idx] = torch.tensor(
                        list(set([r["token_index"] for r in this_matches]))
                    )

        # resid_stream_sample: [batch, n_layers, n_heads, d_model]
        resid_stream_sample, labels = _build_inner_batch(
            recording, filtered_timesteps_not, filtered_timesteps_this
        )
        if len(resid_stream_sample.shape) != 4:
            print("In valid - no inner batch found. skipping.")
            continue
        if resid_stream_sample.shape[0] == 0:
            print("In valid - no inner batch found. skipping.")
            continue
    ...
    all_resid_streams = torch.cat(all_resid_streams, dim=0)
    assert len(all_labels) == all_resid_streams.shape[0]
...
def run_eval(
    ...
    for batch_idx in range(0, valid_size, batch_size):
        ...
        curr_batch_size = resid_stream_sample.shape[0]
        labels_one_hot = F.one_hot(labels, num_classes=2)

        probe_out = einsum(
            "batch n_layers n_heads d_model, n_layers n_heads d_model options -> batch n_layers n_heads options",
            resid_stream_sample,
            probe,
        )

        # labels_one_hot: [batch, options] --> [batch, n_layers, n_heads, options]
        _labels_one_hot = (
            labels_one_hot.unsqueeze(1).unsqueeze(1).repeat((1, n_layers, n_heads, 1))
        )

        # probe_log_probs: [batch, n_layers, n_heads, options]
        probe_log_probs = probe_out.log_softmax(-1)
        probe_correct_log_probs = (
            einops.reduce(
                probe_log_probs * _labels_one_hot.cuda(),
                "batch n_layers n_heads options -> n_layers n_heads options",
                "mean",
            )
            * 2
        )
        ...
        # val_preds, val_gold: [batch, n_layers, n_heads]
        val_preds = probe_log_probs.argmax(-1)
        val_gold = torch.tensor(labels).unsqueeze(-1).unsqueeze(-1).repeat((1, n_layers, n_heads))

        # val_results: [batch, n_layers, n_heads]
        val_results = val_preds.cpu() == val_gold.cpu()

        # _val_accuracy: [n_layers, n_heads]
        _val_accuracy = (val_results.sum(dim=0) / val_results.shape[0]).cpu()

        val_accuracies.append(_val_accuracy * curr_batch_size)
        valid_seen += curr_batch_size

    val_loss = sum(val_losses) / valid_seen
    val_accuracy_per_layer_per_head = torch.stack(val_accuracies, dim=0).sum(dim=0) / valid_seen

    print(f"  Validation loss: {val_loss}")
    print(f"  Validation accuracy: {val_accuracy_per_layer_per_head}")
    if val_loss < lowest_val_loss:
        ...
        torch.save(
            {
                "val_accuracy": val_accuracy_per_layer_per_head,
            },
            metric_path,
        )
    ...
    return {
        "loss": val_loss,
        "accuracy": val_accuracy_per_layer_per_head,
...
def main(config):
    ...
    for batch_idx, batch in enumerate(train_dataloader):
        ...
        # resid_stream_sample: [batch, n_layers, n_heads, d_model]
        if len(filtered_timesteps_not) == 0 or len(filtered_timesteps_this) == 0:
...
if __name__ == "__main__":
    ...
    probe_timestep = config["probe_timestep"]
    if probe_timestep == "(":
        probe_timestep = "open_paren"

    config["save_path"] = (
        f"probe_checkpoints/probe_{config['probe_module']}_{probe_timestep}"
    )
    ...
    elif probe_module == "attn_per_head":
        config["record_module_names"] = [
            f"model.layers.{idx}.self_attn.hook_attn_out_per_head"
